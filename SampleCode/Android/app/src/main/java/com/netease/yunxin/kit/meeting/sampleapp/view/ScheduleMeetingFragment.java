...
public class ScheduleMeetingFragment extends BaseFragment<FragmentScheduleBinding> {
    ...
    protected void initView() {
        ...
        mAdapter = new ScheduleMeetingAdapter(getActivity(), new ArrayList<>(), mViewModel.passWord, mViewModel.tittle, mViewModel.extraData, mViewModel.roleBindData);
        binding.rvScheduleMeeting.setLayoutManager(
                new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false));
        mAdapter.setHasStableIds(true);
        binding.rvScheduleMeeting.setAdapter(mAdapter);
        binding.rvScheduleMeeting.getRecycledViewPool().setMaxRecycledViews(ScheduleMeetingAdapter.VIEW_TYPE,0);
                    ...
                    case ScheduleMeetingItem.ENABLE_MEETING_LIVE_ACTION:
                        ...
                        if(settingsService.isMeetingLiveEnabled()&& isLiveOn){
                            if (binding.rvScheduleMeeting.getScrollState() == RecyclerView.SCROLL_STATE_IDLE
                                    && (binding.rvScheduleMeeting.isComputingLayout() == false)) {
        ...
        mViewModel.observeItemMutableLiveData(getActivity(), new Observer<NEMeetingItem>() {
            ...
            public void onChanged(NEMeetingItem neMeetingItem) {
                if (neMeetingItem != null) {
                    ...
                    if(isEditMeeting){
                        mViewModel.editMeeting(neMeetingItem,new ToastCallback<NEMeetingItem>(getActivity(),"editMeeting"){
                            @Override
    ...
    protected void initData() {
        ...
        handleArguments();
        dataList.add(new ScheduleMeetingItem("会议主题", ScheduleMeetingItem.EDIT_TEXT_TITLE_ACTION, item != null ? item.getSubject() : ""));
        dataList.add(new ScheduleMeetingItem("开始时间", "请选择开始入会时间", ScheduleMeetingItem.SET_START_TIME_ACTION, item != null ? formatTime(item.getStartTime()) : "") );
        dataList.add(new ScheduleMeetingItem("结束时间", "请选择会议结束时间", ScheduleMeetingItem.SET_END_TIME_ACTION, item != null ? formatTime(item.getEndTime()) : ""));
        dataList.add(new ScheduleMeetingItem("会议密码", (item != null && !item.getPassword().isEmpty()), ScheduleMeetingItem.ENABLE_MEETING_PWD_ACTION, item!=null?item.getPassword():""));
        handleAudioSetting();
        handleVideoSetting();
        handleLiveSetting();
        dataList.add(new ScheduleMeetingItem("开启录制", (item != null && item.getSetting().cloudRecordOn), ScheduleMeetingItem.ENABLE_MEETING_RECORD_ACTION,""));
        dataList.add(new ScheduleMeetingItem("扩展字段", ScheduleMeetingItem.SET_EXTRA_DATA_ACTION, item != null ? item.getExtraData() : ""));
        handleRoleBind();
        handleLiveLevel();
    ...
    private void handleLiveLevel() {
        if(item!=null && item.getLive().isEnable()){
            dataList.add(new ScheduleMeetingItem("仅本企业员工可观看", (item!=null && item.getLive().getLiveWebAccessControlLevel() ==
    ...
    private void handleLiveSetting() {
        ...
        if (settingsService.isMeetingLiveEnabled()) {
            dataList.add(new ScheduleMeetingItem("开启直播", (item != null && item.getLive().isEnable()), ScheduleMeetingItem.ENABLE_MEETING_LIVE_ACTION,""));
        }
    }
    ...
    private void handleRoleBind() {
        ...
        if (item != null) {
            ...
            try {
                ...
                dataList.add(new ScheduleMeetingItem("角色绑定", ScheduleMeetingItem.SET_ROLE_BIND, roleBindsJson!=null?roleBindsJson.toString():""));
            }
        } else {
    ...
    private void handleVideoSetting() {
        ...
        if(item!=null){
            isVideoSwitchOn =  item.getSetting().getCurrentVideoControl().getAttendeeOff() != NEMeetingAttendeeOffType.None ;
            videoOffAllowSelfOn = item.getSetting().getCurrentVideoControl().getAttendeeOff() == NEMeetingAttendeeOffType.OffAllowSelfOn;
        }
        dataList.add(new ScheduleMeetingItem("自动关闭视频", "参会者加入会议时自动关闭视频", isVideoSwitchOn, ScheduleMeetingItem.SET_VIDEO_MUTE_ACTION));
        dataList.add(new ScheduleMeetingItem("允许自行打开视频", "参会者被自动关闭视频后可自行打开", videoOffAllowSelfOn, ScheduleMeetingItem.SET_ALLOW_VIDEO_ON_ACTION));
    }
    ...
    private void handleAudioSetting() {
        ...
        if(item!=null){
           isAudioSwitchOn =  item.getSetting().getCurrentAudioControl().getAttendeeOff() != NEMeetingAttendeeOffType.None ;
            audioOffAllowSelfOn = item.getSetting().getCurrentAudioControl().getAttendeeOff() == NEMeetingAttendeeOffType.OffAllowSelfOn;
        }
        dataList.add(new ScheduleMeetingItem("自动静音", "参会者加入会议时自动静音", isAudioSwitchOn, ScheduleMeetingItem.SET_AUDIO_MUTE_ACTION));
        dataList.add(new ScheduleMeetingItem("允许自行解除静音", "参会者被自动静音后可自行打开", audioOffAllowSelfOn, ScheduleMeetingItem.SET_ALLOW_AUDIO_ON_ACTION));
    }
    ...
    private void handleArguments() {
        ...
        if (item != null) {
            ...
            isAttendeeAudioOff =  item.getSetting().getCurrentAudioControl().getAttendeeOff() != NEMeetingAttendeeOffType.None ;
            isAllowAttendeeAudioSelfOn = item.getSetting().getCurrentAudioControl().getAttendeeOff() == NEMeetingAttendeeOffType.OffAllowSelfOn;
            isAttendeeVideoOff =  item.getSetting().getCurrentVideoControl().getAttendeeOff() != NEMeetingAttendeeOffType.None ;
            isAllowAttendeeVideoSelfOn = item.getSetting().getCurrentVideoControl().getAttendeeOff() == NEMeetingAttendeeOffType.OffAllowSelfOn;
            isEditMeeting = true;
        }
    }
