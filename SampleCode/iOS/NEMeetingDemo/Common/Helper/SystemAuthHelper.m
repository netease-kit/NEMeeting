...
+ (BOOL)checkAuthority:(AVAuthorizationStatus)_status {
  return (_status == AVAuthorizationStatusAuthorized) ||
         (_status == AVAuthorizationStatusNotDetermined);
}

+ (void)showAlertController:(AuthorizedFinishBlock)block device:(NSString *)device {
  UIAlertController *alertC = [UIAlertController
      alertControllerWithTitle:@"没有权限"
                       message:[NSString
                                   stringWithFormat:@"请开启‘%@’对 %@ 的使用权限", kAPPName, device]
...
+ (void)cameraAuthorityCheckSuccess:(AuthorizedFinishBlock)success fail:(AuthorizedFinishBlock)fail;
{
...
+ (void)audioAuthorityCheckSuccess:(AuthorizedFinishBlock)success fail:(AuthorizedFinishBlock)fail {
