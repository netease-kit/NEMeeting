...
- (void)buttonClick:(UIButton *)button {
  ...
  if (self.delegate &&
      [self.delegate respondsToSelector:@selector(checkBoxItemdidSelected:atIndex:checkBox:)]) {
    [self.delegate checkBoxItemdidSelected:button
                                   atIndex:[_buttonArray indexOfObject:button]
                                  checkBox:self];
  }
}
...
- (void)refreshButtonFrame {
  ...
  for (UIButton *button in _buttonArray) {
    ...
    [button setFrame:CGRectMake((index % columnCount) * width, (index / columnCount) * height,
                                width, height)];
