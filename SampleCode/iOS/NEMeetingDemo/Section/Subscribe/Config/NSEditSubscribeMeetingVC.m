...
+ (instancetype)editWithItem:(NEMeetingItem *)item {
    ...
    vc.allowAudioSelfOn = item.settings.currentAudioControl.attendeeOff == AttendeeOffTypeOffAllowSelfOn;
    vc.allowVideoSelfOn = item.settings.currentVideoControl.attendeeOff == AttendeeOffTypeOffAllowSelfOn;
    return vc;
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (NEFromGroup *)timeGroup {
    __weak typeof(self) weakSelf = self;
    NEFromGroup *group = [NEFromGroup new];
    NEFromRow *startTimeRow = [NEFromRow rowWithType:NEFromRowTypeTitleDate
                                                 tag:@"kMeetingStartTime"];
...
- (NEFromGroup *)liveGroup {
    __weak typeof(self) weakSelf = self;
    NEFromGroup *group = [NEFromGroup new];
    NEFromRow *liveRow = [NEFromRow rowWithType:NEFromRowTypeTitleSwitch
                                            tag:@"kMeetingLive"];
    liveRow.title = @"开启直播";
...
- (NEFromRow *)liveLevelRow {
    ...
    liveLevelRow.onValueChanged = ^(id  _Nonnull newValue, NEFromRow * _Nonnull row) {
        weakSelf.item.live.liveWebAccessControlLevel = [newValue boolValue] ? NEMeetingLiveAuthLevelAppToken : NEMeetingLiveAuthLevelToken;
    };
...
- (void)commitAction {
    if (self.muteAllAudio || self.muteAllVideo) {
        ...
        if (self.muteAllAudio) {
            ...
            control.attendeeOff = self.allowAudioSelfOn ? AttendeeOffTypeOffAllowSelfOn : AttendeeOffTypeOffNotAllowSelfOn;
            [controls addObject: control];
        }
        if (self.muteAllVideo) {
            NEMeetingControl *control = [NEMeetingControl createVideoControl];
            control.attendeeOff = self.allowVideoSelfOn ? AttendeeOffTypeOffAllowSelfOn : AttendeeOffTypeOffNotAllowSelfOn;
            [controls addObject: control];
        }
    ...
    [[[NEMeetingKit getInstance] getPreMeetingService] editMeeting:self.item
        ...
        if (resultCode == ERROR_CODE_SUCCESS) {
            ...
            [NSNotificationCenter.defaultCenter postNotificationName:kNEMeetingEditSubscribeDone object:nil];
        } else {
