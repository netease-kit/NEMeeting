...
- (void)setupForm {
  ...
  NSMutableArray *dataArray =
      @[ [self titleGroup], [self meetingIDGroup], [self timeGroup] ].mutableCopy;
  // 直播密码
  ...
  if (self.item.live.enable && self.item.live.liveUrl.length) {
    [dataArray addObjectsFromArray:@[ [self liveUrlGroup], [self enterpriseStaffGroup] ]];
  }
  // 扩展字段
...
- (NEFromGroup *)enterpriseStaffGroup {
  NEFromGroup *group = [NEFromGroup new];
  NEFromRow *meetingLiveLevelRow = [NEFromRow rowWithType:NEFromRowTypeTitleSwitch
                                                      tag:@"kMeetingLiveLevel"];
  meetingLiveLevelRow.title = @"仅本企业员工可观看";
  meetingLiveLevelRow.subTitle =
      _item.live.liveWebAccessControlLevel == NEMeetingLiveAuthLevelAppToken ? @"已开启"
                                                                             : @"未开启";
  meetingLiveLevelRow.value =
      _item.live.liveWebAccessControlLevel == NEMeetingLiveAuthLevelAppToken ? @(YES) : @(NO);
...
- (NEFromGroup *)extraDataGroup {
  NEFromGroup *group = [NEFromGroup new];
  NEFromRow *extraDataRow = [NEFromRow rowWithType:NEFromRowTypeTitleInput
...
- (NEFromGroup *)muteAudioGroup {
  ...
  allowSelfOnRow.value =
      @(_item.settings.currentAudioControl.attendeeOff == AttendeeOffTypeOffAllowSelfOn);
...
- (NEFromGroup *)muteVideoGroup {
  ...
  allowSelfOnRow.value =
      @(_item.settings.currentVideoControl.attendeeOff == AttendeeOffTypeOffAllowSelfOn);
  allowSelfOnRow.userInteractionEnabled = NO;
...
- (void)doCancelMeeting {
  ...
  [[self preMeetingService]
               cancelMeeting:_item.meetingId
      cancelRecurringMeeting:NO
                    callback:^(NSInteger resultCode, NSString *resultMsg) {
                      if (resultCode == ERROR_CODE_SUCCESS) {
                        ...
                        NSString *msg =
                            [NSString stringWithFormat:@"%@:%@", resultMsg, @(resultCode)];
...
- (void)doJoinMeeting {
  ...
  watermarkConfig.name = [self accountName];
...
- (NSString *)accountName {
  ...
  if (ret.length == 0) ret = [self displayName];
  return ret;
}
...
- (UIButton *)cancelMeetingBtn {
  if (!_cancelMeetingBtn) {
    ...
    [_cancelMeetingBtn addTarget:self
                          action:@selector(doCancelMeetingAction:)
...
- (UIButton *)joinMeetingBtn {
  if (!_joinMeetingBtn) {
    ...
    [_joinMeetingBtn addTarget:self
                        action:@selector(doJoinMeetingAction:)
              forControlEvents:UIControlEventTouchUpInside];
  }
